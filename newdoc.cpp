    ...
    ui(new Ui::newdoc)
{
    ...
    model->setHeaderData(0,Qt::Horizontal,QString::fromUtf8("代保管物品名称"));
    model->setHeaderData(1,Qt::Horizontal,QString::fromUtf8("代保管物品号码"));
...
void newdoc::on_pushButton_3_clicked()//录入按钮事件
{
   bool query_OK=0;
    ...
    query_OK= query.exec("create table mys(hetonhao1 vchar primary key,weituobumen2 vchar ,weituoren3 vchar ,baoguanyapingming4 vchar ,baoguanyapinghao5 vchar ,baoguanwujiazhi6 vchar ,wupingliebiao7 vchar ,dianzibiaoqian8 vchar,rq9 vchar)");
    if(query_OK)
    {
    ...
    if(NumberRow>0&&NumberColumn>0)
    {
        for(int h=0;h<NumberRow;h++)
        {
                ...
                //h1l1+&h1l2|h2l1+&h2l2
                //遍历第row行的所有信息
                 QModelIndex index = model->index(h,0);
                 QString nr = model->data(index).toString();

                 index = model->index(h,1);
                 nr = nr+","+model->data(index).toString();
                 if(h<NumberRow-1)nr+="\n";
                 wupingliebiao7+=nr;
...
void newdoc::on_pushButton_5_clicked()//添加表单
{
        ...
        model->setItem(_rowCount,1,item);
